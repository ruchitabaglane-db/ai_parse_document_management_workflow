...
CREATE  STREAMING TABLE `databricks_sandbox`.`development`.`KIE_ocr_endpoint_responses` 
  TBLPROPERTIES (
  ...
  WITH query_results AS (
    ...
    FROM STREAM(`databricks_sandbox`.`development`.`raw_ocr_pdf_files`) 
  )
  SELECT
    input,
    source_file_path,
...
CREATE  STREAMING TABLE databricks_sandbox.development.ocr_canonical_invoices
  (
    input STRING,
...
WITH parsed_responses AS (
  ...
  FROM STREAM(`databricks_sandbox`.`development`.`KIE_ocr_endpoint_responses`)
